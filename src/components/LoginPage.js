...
function LoginPage() {
    ...
    return (
        <Flex zIndex={1} justifyContent={"center"} alignItems={"center"} width={"100%"} height={"100vh"} bgColor={"rgb(240,240,240)"}>
            <Flex id="main-block" direction={"column"} gap={2} bgColor={"white"} padding={"80px 30px"} borderRadius={10} alignItems={'center'}>
                ...
                <Button type="button" width={300} onClick={async ()=>{
                    ...
                    if (result.length > 0) {
                        ...
                        window.sessionStorage.setItem("taskName",(result[0].task_name));
                        navigate("Home");
                    } else {
                        setWrong(true);
                    }
