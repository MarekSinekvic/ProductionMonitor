...
import { AlertDialog, AlertDialogBody, AlertDialogContent, AlertDialogFooter, AlertDialogHeader, AlertDialogOverlay, Avatar, Button, CloseButton, Divider, Drawer, DrawerBody, DrawerCloseButton, DrawerContent, DrawerHeader, DrawerOverlay, Flex, Heading, Input, Stack, StackDivider, Text, Textarea, useDisclosure } from "@chakra-ui/react";
...
export default function ChatTab({messageCountSetter}) {
    ...
    function updateSendersList() {
        (async ()=>{
            ...
            // let _users = await window.DB.getGeneralRaw(`
            //     select users.*,mes.*,files_links.file_path as avatar_url from users
            ...
            //     left join files_links on users.avatar_file_id=files_links.id
            //     ${searchSQL}
            //     order by date desc
            // `);
            let _users = await window.DB.getGeneralRaw(`
                select users.*,mes1.last_message,mes2.mess_count,files_links.file_path as avatar_url from users
                left join (select mes1.sender_id,mes1.message as last_message,mes1.date as last_date from users_messages as mes1
                            join (select max(date) as date from users_messages where receiver_id=${selfId} group by sender_id) as mes2 on mes2.date=mes1.date) as mes1 on mes1.sender_id=users.id
                left join (select sender_id,count(case when state = 0 then 1 else null end) as mess_count from users_messages where receiver_id=${selfId} group by sender_id) as mes2 on mes2.sender_id=users.id
                left join files_links on users.avatar_file_id=files_links.id
                ${searchSQL}
                    ...
                    <TabPanel p={0}>
                        <Stack divider={<StackDivider/>} cursor={'pointer'} marginTop={'10px'}>
                            {users.map((user,id)=>{ //.filter((user)=>{return user.id!=Number(window.sessionStorage.getItem('id'))})
                                //console.log(user);
                                return ( //  borderLeft={(user.mess_count != null && user.mess_count > 0) ? '2px solid red' : ''}
                                    <Flex key={id} paddingX={1} direction={'column'} onClick={()=>{getMessagesFromUser(user.id); setViewTarget({target:'users',index:user.id});}}>
                                        <Heading color={(viewTarget.index==user.id ? 'rgb(180,40,40,1)' : 'black')} size={'xs'} textTransform={'uppercase'}>
                                            <Flex>
                                                <Flex w={'100%'} direction={'row'} gap={2} alignItems={'center'}>
                                                    <Avatar src={"img:///"+user.avatar_url} size={'sm'}></Avatar>
                                                    <Flex w={'100%'} direction={'column'} color={(user.mess_count != null && user.mess_count > 0) ? 'rgb(220,0,0)' : ''}>
                                                        <Flex>{user.name}</Flex>
                                                        <Text fontWeight={500} fontSize={'9px'} overflow={'hidden'} whiteSpace={'nowrap'} w={'80%'}>Last message: {user.last_message}</Text>
                                                    </Flex>
                                                </Flex>
                                ...
                                return (
                                    <Flex key={id} direction={'column'} onClick={()=>{getMessagesFromGroup(group.id); setViewTarget({target:'groups',index:group.id});}}>
                                        <Heading color={(viewTarget.index==group.id ? 'rgb(180,40,40,1)' : 'black')} size={'xs'} textTransform={'uppercase'}>
                                            <Flex color={(group.mess_count != null && group.mess_count > 0) ? 'rgb(220,0,0)' : ''}>{group.name}</Flex>
                        ...
                        <Flex key={id} direction={'column'} gap={'5px'}>
                            <Flex direction={'column'} alignItems={message.sender_id == selfId ? 'flex-end' : 'flex-start'}>
                                <Flex alignItems={'center'}>
                                    ...
                                    <Popover size={'xs'}>
                                        <PopoverTrigger>
                                            <div style={{width:'8px', height:'8px', backgroundColor: message.state == 0 ? 'rgb(120,120,255)' : 'green', borderRadius:'50%', marginRight:'5px'}}></div>
                                        </PopoverTrigger>
                                        <PopoverContent>
                                            <PopoverHeader>
                                                Information
                                            ...
                                            <PopoverBody>
                                                <Text as={'i'}>
                                                    {message.state == 0 ? "message not readed" : ""}
                                                </Text>
